  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 mb-8">
        <h3 className="text-lg font-medium text-slate-800 mb-4">Nova Categoria</h3>
        <form onSubmit={handleCreate} className="flex gap-4">
          <input 
            ...
            className="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2 text-slate-900 focus:outline-none focus:border-brand"
          />
          <button 
            type="submit"
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-200">
            {loading ? (
              ...
              <tr>
                <td colSpan={2} className="px-6 py-8 text-center text-slate-600">Nenhuma categoria encontrada.</td>
              </tr>
            ) : (
              categories.map(cat => (
                  ...
                  <td className="px-6 py-4">
                    {editingId === cat.id ? (
                      <input 
                        ...
                        className="bg-white border border-brand/50 rounded px-3 py-1 text-slate-900 focus:outline-none focus:border-brand w-full max-w-sm"
                        autoFocus
                        onKeyDown={(e) => {
                    ...
                    <div className="flex justify-end gap-2">
                      {editingId === cat.id ? (
                        <>
                          ...
                          <button onClick={() => setEditingId(null)} className="p-2 text-slate-600 hover:bg-white rounded-lg transition-colors" title="Cancelar">
                            <X size={18} />
                          </button>
                        </>
                      ) : (
